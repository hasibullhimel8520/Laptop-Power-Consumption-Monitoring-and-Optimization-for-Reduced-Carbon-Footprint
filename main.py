...
# Try brightness control (may fail gracefully)
try:
...
DB_LIVE = "power_live.db"
DB_SIM = "power_sim_5days.db"
...
def collector_thread():
    """Collect real laptop data for LIVE mode."""
    ...
    if mode == "live":
        ...
        cur.execute(
            """SELECT timestamp,cpu_util,brightness,battery_percent,is_plugged,power_W,energy_Wh_cum
                   FROM samples
                   WHERE timestamp >= ?
                   ORDER BY timestamp""", (since.isoformat(timespec="seconds"),)
        )
    else:
...
def api_summary():
    ...
    cost = energy_kWh * COST_PER_KWH_BDT
